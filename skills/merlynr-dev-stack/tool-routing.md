...
| **Claude** | MCP memory | Built-in `grep` → `find` → `cat` | Built-in web search MCP / Fetch |
| **Antigravity** | MCP `read_working_memory` / `memory_search` | Native `grep_search` → `list_dir` → `view_file` | Native `search_web` → `read_url_content` |

```plaintext
Cursor   → 不要默认 Cymbal；无 Cymbal 时用 SemanticSearch + Grep
Codex    → Cymbal 优先于裸 Grep（符号/调用链）
OpenCode → explore 并行摸底，再精确 Grep
Claude / Antigravity / OpenCode → 记忆读写必须 100% 优先调用 MCP Tool (如 memory_add, memory_search, read_working_memory)，禁止在有 MCP 挂载的环境下通过终端运行 nmem 命令行，以彻底避免 Windows 终端下的中文乱码漏洞。
```
...
| 时机 | 方式（OpenCode/Claude/Antigravity） | 方式（裸 CLI / Codex 环境） |
|------|------------------------------------|----------------------------|
| 会话结束（>30s 有结论） | MCP `memory_add` | `distill-memory` → `nmem m add/update` |
| **M 级任务完成**（有结论） | Phase 4.5 模块 agent **必须**；nmem **建议** ≥1 条 | 同左，使用 CLI 写入 |
| **L 级任务完成** | Phase 4.5 **必须** + MCP `memory_add` **必须** ≥1 条 | 同左，使用 CLI 写入 |
| 里程碑/架构决策 | MCP `memory_add` | `distill-memory` |
| GSD 阶段完成 | `gsd-extract-learnings` | `gsd-extract-learnings` |
| gsd-team 团队任务 | 执行 agent 收尾 MCP `memory_add` | 执行 agent 收尾 `--learn` |

写入由 Agent 按 skill 执行；**不会**因改代码或 commit 自动入库。

**UTF-8 安全与乱码防范**：
- **MCP环境（Claude, Antigravity, OpenCode）**：**强制**使用 MCP `memory_add` 等接口写入，数据天然通过 JSON 传输，100% UTF-8 安全。
- **裸CLI环境（Codex / Windows PowerShell）**：
  若由于环境限制不得不使用 CLI 命令行，应使用以下方式以防控制台编码问题：
  ```powershell
